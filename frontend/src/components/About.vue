<template>
  ...
  >
    <div
      class="rounded-xl p-6 w-80 max-w-[90vw] relative"
    ...
    >
      ...
      <div class="text-center mb-4">
        <h2 class="text-xl font-bold" :style="{ color: 'var(--accent)' }">mdView</h2>
        <p class="text-xs opacity-50 mt-1">v1.0.0</p>
      </div>

      <p class="text-sm text-center opacity-70 mb-4 leading-relaxed">
        {{ t.aboutDesc }}
      </p>

      <div class="text-sm opacity-60 mb-5 space-y-2">
        <div class="flex justify-between">
          <span>{{ t.aboutFramework }}</span>
          <span :style="{ color: 'var(--accent)' }">Wails v2 + Go</span>
        </div>
        <div class="flex justify-between">
          <span>{{ t.aboutUI }}</span>
          <span :style="{ color: 'var(--accent)' }">Vue 3 + Tailwind</span>
        </div>
        <div class="flex justify-between">
          <span>{{ t.aboutEditor }}</span>
          <span :style="{ color: 'var(--accent)' }">CodeMirror 6</span>
        </div>
      </div>

      <div class="flex gap-2 justify-center">
        <button
          class="toolbar-btn text-xs flex items-center gap-1"
          @click="openURL('https://github.com/PerfLite')"
        >
        ...
        <button
          class="toolbar-btn text-xs flex items-center gap-1"
          @click="openURL('https://t.me/bashakul')"
        >
